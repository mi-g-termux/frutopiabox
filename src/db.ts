...
export function getActiveEngine(): DatabaseEngine {
  try {
    ...
    // No engine key set yet — auto-detect if Firebase credentials exist.
    // This prevents the app from falling back to localStorage after the
    // Install Wizard has configured Firebase.
...
export const DEFAULT_ADMIN_CREDENTIALS: AdminCredentials = {
  username: '', password: '', googleSignInEnabled: false, googleClientId: '',
};
...
export const dbService = {
  ...
  async saveAdminSettings(settings: AdminCredentials): Promise<void> {
    ...
    if (fbOk()) { try { await setDoc(doc(getDb()!, 'settings', 'adminSettings'), settings); } catch (e) { console.warn('[db] Firestore saveAdminSettings failed (local save ok):', e); } }
  },
