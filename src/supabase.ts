 ...
 * Callback fires with the full `new` row whenever the `settings` table
 * row with id='siteSettings' is updated.
 */
...
// ── Attach Realtime subscription for siteSettings ────────────────────────────
    ...
    _settingsChannel = _client
      ...
      .on(
        'postgres_changes',
        {
          ...
          schema: 'public',
          table: 'settings',
          filter: "key=eq.siteSettings",
        },
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        (payload: any) => {
          console.log('[Supabase] siteSettings updated via Realtime:', payload);
          _notifySettings(payload?.new);
        },
      )
...
// This mirrors firebase.ts's boot-on-import pattern so both engines
// restore themselves across page refreshes automatically.
(async () => {
  const saved = getSupabaseRuntimeConfig();
  if (saved) {
